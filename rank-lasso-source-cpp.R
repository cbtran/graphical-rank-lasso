...
rank_lasso_solver <- function(x, y, alpha0=0.1, c=1.01, B=500, 
    ...
    if (m > 0) {
        ...
        sub_sample_indx <- 1:(n*(n-1)/2)
    }
    
    X_tilde <- pair.diff(x)
    y_tilde <- c(pair.diff(matrix(y)))
    ...
    if (solver=="lpsolve") {
        constr_mat_list <- create_constr_mat_dense(X_tilde[sub_sample_indx,], num_terms)
        f.con <- constr_mat_list$constr_mat
        
    } else {
        constr_mat_list <- create_constr_mat(X_tilde[sub_sample_indx,], num_terms)
        f.con <- constr_mat_list$constr_mat
    }
    
    # Set right hand side coefficients
    f.rhs.lwr <- c(y_tilde[sub_sample_indx], rep(0, 2*p))
    ...
    lambda.list <- list()
    for (i in 1:n.eta){
        lambda.list[[i]] <- dmcp(abs(beta_rank), eta.list[i], 3)
    }
